...
from ... import data
from ...seq import Alphabet, Seq, protein_alphabet
    ...
    for n, m in matrices[1:]:  # add all other matrices
        ...
        if m.taxsets:
            ...
            combined.taxsets.update(
                dict(("%s.%s" % (n, tn), ts) for tn, ts in m.taxsets.items())
            )
        ...
        if m.charlabels:
            ...
            combined.charlabels.update(
                dict((combined.nchar + i, label) for (i, label) in m.charlabels.items())
            )
...
class Nexus(object):
                ...
                if self.matchchar:
                    while True:
                        ...
                        if p == -1:
                            break
                        iupac_seq = Seq(
                            str(iupac_seq)[:p] + refseq[p] + str(iupac_seq)[p + 1 :],
                            self.alphabet,
                        )
    ...
    def crop_matrix(self, matrix=None, delete=[], exclude=[]):
        ...
        if exclude != []:
            ...
            return dict(
                (t, matrix[t])
                for t in self.taxlabels
                if t in matrix and t not in delete
            )
    ...
    def add_sequence(self, name, sequence):
        ...
        assert (
            unique_name not in self.matrix
    ...
    def terminal_gap_to_missing(self, missing=None, skip_n=True):
        ...
        for taxon in self.taxlabels:
            ...
            assert length == len(sequence), (
                "Illegal sequence manipulation in Nexus.terminal_gap_to_missing in taxon %s"
                % taxon
            )
